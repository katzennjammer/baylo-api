...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
     ...
     * staff (ADMIN only) and has no database to ask. It is NOT what /admin
     * trusts -- that layout and every /api/admin route read the column on each
     * request, so a revoked admin loses /admin the moment the row changes.
     * What a stale claim can leak is the retired USER dashboard, a UI over the
     * same API the phone already gives every account; nobody gains a permission
     * from it.
     *
